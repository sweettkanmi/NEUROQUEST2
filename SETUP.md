...
Go to **Authentication → Settings** → turn off **"Confirm email"** so users can log in immediately after registration without email verification. This is the expected behavior for this application.
...
3. You will be logged in immediately and redirected to the dashboard
...
| User created but can't log in | Make sure "Confirm email" is disabled in Supabase → Authentication → Settings |
